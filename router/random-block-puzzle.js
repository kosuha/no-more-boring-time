...
router.post("/random-block-puzzle", (request, response) => {
...
router.post("/user-data-process", (request, response) => {
    ...
    if (userData === undefined) {
        response.json({ auth: false, userData: 0, highest: 0 });
    } else {
        connection.query(
            `SELECT * FROM rank_random_block_puzzle WHERE id_kakao = ${userData.id}`,
            (error, rows, fields) => {
                if (error) {
                    throw error;
                }
                if (Object.keys(rows).length === 0) {
                    response.json({ auth: true, userData: userData, highest: 0 });
                } else {
                    response.json({ auth: true, userData: userData, highest: rows[0].score });
                }
            }
        );
    }
...
router.post("/ranking-process", (request, response) => {
    connection.query(
        "SELECT * FROM rank_random_block_puzzle ORDER BY score DESC LIMIT 10",
        (error, rows, fields) => {
            response.json(rows);
        }
    );
